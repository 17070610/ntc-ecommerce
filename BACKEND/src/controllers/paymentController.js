...
export const initiatePayment = async (req, res) => {
  try {
    ...
    if (!phoneNumber || !amount || !orderId) {
      return res.status(400).json({ success: false, message: 'Phone number, amount, and order ID are required' });
    }

    if (!mtnMomoService.validatePhoneNumber(phoneNumber)) {
      return res.status(400).json({ success: false, message: 'Invalid phone number format. Use 250XXXXXXXXX or 07XXXXXXXX' });
    }
    ...
    if (!accountStatus.result) {
      return res.status(400).json({ success: false, message: 'Phone number is not registered for MTN MoMo' });
    }

    const paymentResult = await mtnMomoService.requestPayment(normalizedPhone, amount, orderId, message || `Payment for order ${orderId}`);
    res.status(200).json({ success: true, message: 'Payment initiated successfully', data: paymentResult });
  } catch (error) {
...
export const getAccountBalance = async (req, res) => {
  try {
    const balance = { availableBalance: '1000000', currency: 'RWF', mockMode: true };
...
export const validatePhoneNumber = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      data: { phoneNumber: normalizedPhone, isValid: true, isActive: accountStatus.result }
    });
  } catch (error) {
    console.error('Phone validation error:', error);
    res.status(500).json({ success: false, message: error.message || 'Failed to validate phone number' });
  }
};
